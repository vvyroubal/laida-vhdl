...
sim:
	...
	@mkdir -p $(BUILD)/02 && cd $(BUILD)/02 && \
	  ...
	  ghdl -r tb_binary_to_gray --vcd=tb_binary_to_gray.vcd
	...
	@mkdir -p $(BUILD)/03 && cd $(BUILD)/03 && \
	  ...
	  ghdl -r tb_logic_function --vcd=tb_logic_function.vcd

	@echo "[03] majority"
	@mkdir -p $(BUILD)/03 && cd $(BUILD)/03 && \
	  ghdl -a ../../03/majority.vhd ../../03/tb_majority.vhd && \
	  ghdl -e tb_majority && \
	  ghdl -r tb_majority --vcd=tb_majority.vcd
	...
	@mkdir -p $(BUILD)/04 && cd $(BUILD)/04 && \
	  ...
	  ghdl -r tb_minimized_function --vcd=tb_minimized_function.vcd
	...
	@mkdir -p $(BUILD)/08 && cd $(BUILD)/08 && \
	  ...
	  ghdl -r tb_sequence_detector --vcd=tb_sequence_detector.vcd
	...
	@mkdir -p $(BUILD)/09/binary_counter && cd $(BUILD)/09/binary_counter && \
	  ...
	  ghdl -r tb_binary_counter --vcd=tb_binary_counter.vcd
	...
	@mkdir -p $(BUILD)/09/async_binary_counter && cd $(BUILD)/09/async_binary_counter && \
	  ...
	  ghdl -r tb_async_binary_counter --vcd=tb_async_binary_counter.vcd
	...
	@mkdir -p $(BUILD)/09/ring_counter && cd $(BUILD)/09/ring_counter && \
	  ...
	  ghdl -r tb_ring_counter --vcd=tb_ring_counter.vcd
	...
	@mkdir -p $(BUILD)/09/johnson_counter && cd $(BUILD)/09/johnson_counter && \
	  ...
	  ghdl -r tb_johnson_counter --vcd=tb_johnson_counter.vcd
	...
	@mkdir -p $(BUILD)/09/modulo_counter && cd $(BUILD)/09/modulo_counter && \
	  ...
	  ghdl -r tb_modulo_counter --vcd=tb_modulo_counter.vcd

	@echo "[09] seq_gen"
	@mkdir -p $(BUILD)/09/seq_gen && cd $(BUILD)/09/seq_gen && \
	  ghdl -a ../../../09/seq_gen.vhd ../../../09/tb_seq_gen.vhd && \
	  ghdl -e tb_seq_gen && \
	  ghdl -r tb_seq_gen --vcd=tb_seq_gen.vcd
