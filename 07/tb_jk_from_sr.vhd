...
begin
    ...
    begin
        ...
        j <= '0'; k <= '0';
        wait for period;
        assert q = '0' report "JK=00 (Q je bio 0), očekivano Q=0" severity error;

        -- JK=10: postavljanje (Q -> 1)
        j <= '1'; k <= '0';
        wait for period;
        assert q = '1' report "JK=10, očekivano Q=1" severity error;

        -- JK=00: zadržavanje (Q=1)
        j <= '0'; k <= '0';
        wait for period;
        assert q = '1' report "JK=00 (Q je bio 1), očekivano Q=1" severity error;

        -- JK=01: brisanje (Q -> 0)
        j <= '0'; k <= '1';
        wait for period;
        assert q = '0' report "JK=01, očekivano Q=0" severity error;

        -- JK=11: promjena stanja (Q -> 1)
        j <= '1'; k <= '1';
        wait for period;
        assert q = '1' report "JK=11 (Q je bio 0), očekivano Q=1" severity error;

        -- JK=11: promjena stanja (Q -> 0)
        j <= '1'; k <= '1';
        wait for period;
        assert q = '0' report "JK=11 (Q je bio 1), očekivano Q=0" severity error;
